...
model_str <- paste0('-vmPFC-HC-network-',toalign,'-ranslopes-nofixedeffect-noHCbetween-',2,'.Rdata') #3'rd iteration is HCwithin
...
Q2 <- Q2 %>% group_by(id) %>% mutate(slrs_within = scale(subj_level_rand_slope)) %>% ungroup()

Q2$subj_level_rand_slope <- scale(Q2$subj_level_rand_slope)

mDMNsex_mmclock_fmri_AH <- lmerTest::lmer(data=Q2 %>% filter(network=='DMN' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mCTRsex_mmclock_fmri_AH <- lmerTest::lmer(data=Q2 %>% filter(network=='CTR' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mLIMsex_mmclock_fmri_AH <- lmerTest::lmer(data=Q2 %>% filter(network=='LIM' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mDMNsex_mmclock_fmri_PH <- lmerTest::lmer(data=Q2 %>% filter(network=='DMN' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id)) 
mCTRsex_mmclock_fmri_PH <- lmerTest::lmer(data=Q2 %>% filter(network=='CTR' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mLIMsex_mmclock_fmri_PH <- lmerTest::lmer(data=Q2 %>% filter(network=='LIM' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
...
Q3 <- Q3 %>% group_by(id) %>% mutate(slrs_within = scale(subj_level_rand_slope)) %>% ungroup()

Q3$subj_level_rand_slope <- scale(Q3$subj_level_rand_slope)

mDMNsex_mmclock_meg_AH <- lmerTest::lmer(data=Q3 %>% filter(network=='DMN' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mCTRsex_mmclock_meg_AH <- lmerTest::lmer(data=Q3 %>% filter(network=='CTR' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mLIMsex_mmclock_meg_AH <- lmerTest::lmer(data=Q3 %>% filter(network=='LIM' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mDMNsex_mmclock_meg_PH <- lmerTest::lmer(data=Q3 %>% filter(network=='DMN' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mCTRsex_mmclock_meg_PH <- lmerTest::lmer(data=Q3 %>% filter(network=='CTR' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mLIMsex_mmclock_meg_PH <- lmerTest::lmer(data=Q3 %>% filter(network=='LIM' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
...
setwd('~/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection/')
model_str <- paste0('Bsocial-vPFC-HC-network-',toalign,'-ranslopes-nofixedeffect-',2,'.Rdata')
model_str <- Sys.glob(paste0('*',model_str))
load(model_str)
...
Qb <- Qb %>% group_by(id) %>% mutate(slrs_within = scale(subj_level_rand_slope)) %>% ungroup()

Qb$subj_level_rand_slope <- scale(Qb$subj_level_rand_slope)

mDMNsex_bsocial_fmri_AH <- lmerTest::lmer(data=Qb %>% filter(network=='DMN' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mCTRsex_bsocial_fmri_AH <- lmerTest::lmer(data=Qb %>% filter(network=='CTR' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mLIMsex_bsocial_fmri_AH <- lmerTest::lmer(data=Qb %>% filter(network=='LIM' & HC_region == 'AH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mDMNsex_bsocial_fmri_PH <- lmerTest::lmer(data=Qb %>% filter(network=='DMN' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mCTRsex_bsocial_fmri_PH <- lmerTest::lmer(data=Qb %>% filter(network=='CTR' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
mLIMsex_bsocial_fmri_PH <- lmerTest::lmer(data=Qb %>% filter(network=='LIM' & HC_region == 'PH'), rt_csv ~ slrs_within*rt_lag_sc + subj_level_rand_slope*rt_lag_sc + (1|id))
