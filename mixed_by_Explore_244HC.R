...
if (do_vPFC_HC){
  ...
  for (i in 1:length(decode_formula)){
    ...
    # )
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Explore-vPFC-HC-network-clock-HConly-244HC-',i,'.Rdata'))
  }
...
if (do_HC_anatomy){
  ...
  for (i in 1:length(decode_formula)){
    ...
    # )
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Explore-vPFC-HC-network-clock-HConly-anatomy-244HC-',i,'.Rdata'))
  }
