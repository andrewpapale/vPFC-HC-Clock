...
if (do_vPFC_clock){
  ...
  if (remove_1to10){
    ...
    rm(decode_formula)
  decode_formula <- formula(~ (1|id))
  decode_formula[[1]] = formula(~ age + v_entropy_wi*age + trial_neg_inv_sc + last_outcome + rt_lag_sc + iti_lag_sc + (1|id/run))
  decode_formula[[2]] = formula(~ age + v_max_wi*age + trial_neg_inv_sc + last_outcome + rt_lag_sc + iti_lag_sc +  (1 |id/run))
  decode_formula[[3]] = formula(~ age + v_entropy_wi + v_max_wi + trial_neg_inv_sc + last_outcome*age + rt_lag_sc + iti_lag_sc + (1|id/run))
  decode_formula[[4]] = formula(~ age + v_entropy_wi + trial_neg_inv_sc*age + last_outcome + rt_lag_sc + iti_lag_sc +  (1 |id/run))
  ...
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-Age-vmPFC-symmetry-clock-',i,'.Rdata'))
    }
  }
  if (do_network){
    splits = c('evt_time','network')
    ...
    for (i in 1:length(decode_formula)){
      ...
      if (i == 1){
        ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = decode_formula[[i]] , split_on = splits,
                        ...
                        emtrends_spec = list(
                          varXage = list(outcome='vmPFC_decon',model_name='model1', var = 'v_entropy_wi',
                                specs=formula(~v_entropy_wi:age),at=list(v_entropy_wi=c(-1.5,1.5),age=c(-1.5,1.5)))
                        )
        )
      } else if (i == 2) {
        ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = decode_formula[[i]] , split_on = splits,
                        ...
                        emtrends_spec = list(
                          varXage = list(outcome='vmPFC_decon',model_name='model1', var = 'v_max_wi',
                                         specs=formula(~v_max_wi:age),at=list(v_max_wi=c(-1.5,1.5),age=c(-1.5,1.5)))
                        )
        )        
      } else if (i == 3){
        ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = decode_formula[[i]] , split_on = splits,
                        ...
                        emtrends_spec = list(
                          varXage = list(outcome='vmPFC_decon',model_name='model1', var = 'trial_neg_inv_sc',
                                         specs=formula(~trial_neg_inv_sc:age),at=list(trial_neg_inv_sc=c(-1.5,1.5),age=c(-1.5,1.5)))
                        )
        )       
      } else if (i == 4){
        ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = decode_formula[[i]] , split_on = splits,
        ...
        curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
        save(ddf,file=paste0(curr_date,'-Age-vmPFC-network-clock-',i,'.Rdata'))
    }
    if (do_vif){
...
if (do_HC_fb){
  ...
  #df <- df %>% filter(!is.na(rt_vmax_change_bin) | !is.na(v_entropy_wi_change_bin))
  df <- df %>% select(id,run,run_trial,v_entropy_wi,v_entropy_wi_change,rt_vmax_change_sc,trial_bin,iti_ideal,iti_prev, rt_csv,rt_csv_sc, trial_neg_inv_sc,rewFunc,v_entropy_sc,outcome,v_max_wi,v_entropy_wi_change,score_sc,rt_bin,iti_sc,ev_sc,expl_longer,expl_shorter)
  ...
  decode_formula[[2]] = formula(~ v_entropy_wi + (1|id/run))
...
if (do_HC_clock){
  ...
  #rm(clock_comb)
  hc <- hc %>% select(!HC_region)
  hc <- hc %>% mutate(HC_region = case_when(bin_num <= 4 ~ 'PH',
                                                  bin_num >= 9 ~ 'AH',
                                                  bin_num >5 & bin_num < 9 ~ 'IH')
  )
  hc <- hc %>% filter(HC_region != 'IH')
  ...
  rm(decode_formula)
  decode_formula <- NULL
  decode_formula[[1]] = formula(~ v_entropy_wi + last_outcome*age + rt_lag_sc + iti_lag_sc + trial_neg_inv_sc + HCbetween + (1|id/run))
  #decode_formula[[2]] = formula(~ v_max_wi*age + last_outcome + rt_lag_sc + iti_lag_sc + trial_neg_inv_sc + HCbetween + (1|id/run))
  #decode_formula[[3]] = formula(~v_entropy_wi*age + (1|id/run))
  #decode_formula[[4]] = formula(~v_max_wi*age + (1|id/run))
  ...
  qT <- c(-0.7,0.43)
  splits = c('evt_time','HC_region','female')
  #source("~/fmri.pipeline/R/mixed_by.R")
  for (i in 1:length(decode_formula)){
    setwd('~/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
    df0 <- decode_formula[[i]]
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Age-HC-MFsplit-poles-region-clock-',i,'.Rdata'))
  }
}
####################################
...
if (do_HC2vPFC_fb){
  ...
  #df <- df %>% filter(!is.na(rt_vmax_change_bin) | !is.na(v_entropy_wi_change_bin))
  df <- df %>% select(total_earnings_split,id,run,run_trial,v_entropy_wi_change,rt_vmax_change_sc,trial_bin,iti_ideal,iti_prev,rt_csv,rewFunc,trial_neg_inv_sc,rt_csv_sc,v_entropy_wi,outcome,v_max_wi,trial_neg_inv_sc,rt_csv_sc,v_entropy_wi_change,score_sc,rt_bin,iti_sc,ev_sc,expl_longer,expl_shorter)
  ...
  decode_formula[[2]] = formula(~ age*HCwithin + female*HCwithin + v_entropy_wi*HCwithin + trial_neg_inv_sc*HCwithin  + rt_csv_sc*HCwithin  + iti_sc*HCwithin + outcome*HCwithin + HCbetween + (1 |id/run))
  decode_formula[[3]] = formula(~age*HCwithin + (1 | id/run))
...
if (do_HC2vPFC_clock){
  ...
  #df <- df %>% filter(!is.na(rt_vmax_change_bin) | !is.na(v_entropy_wi_change_lag_bin))
  df <- df %>% select(id,run,trial,run_trial,rt_lag_sc,total_earnings_split,outcome,iti_ideal,iti_prev,rt_csv,
                      abs_pe_max_lag_sc,v_entropy_wi,rt_vmax_change_sc,trial_bin,rewFunc,trial_neg_inv_sc,
                      rt_csv_sc,v_entropy_sc,expl_longer,expl_shorter,rt_bin,trial_bin,last_outcome,v_max_wi,
                      v_entropy_wi_change_lag,score_lag_sc,iti_sc,iti_lag_sc,ev_lag_sc,
  ...
  rm(decode_formula)
  decode_formula <- NULL
  #decode_formula[[1]] = formula(~ female + v_entropy_wi * HCwithin * age + trial_neg_inv_sc + v_max_wi * HCwithin * age + rt_csv_sc + iti_lag_sc + iti_sc + last_outcome + HCbetween + (1 | id/run)) 
  ...
  #decode_formula[[2]] = formula(~age * HCwithin *v_entropy_wi + HCbetween + (1 | id/run)) 
