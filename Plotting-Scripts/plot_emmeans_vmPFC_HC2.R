...
plot_emmeans_vmPFC_HC2 <- function(ddf,toalign,toprocess,totest,behavmodel,model_iter,hc_LorR){
  ...
  fname = paste(behavmodel,'-',totest,"_",toalign, "_emmeans_", toprocess, "_", 'Entropy-by-HC2','-',hc_LorR, ".pdf", sep = "")
  pdf(fname, width = 9, height = 3.5)
  gg1 <- ggplot(emt,aes(x=evt_time,y=estimate)) + 
    facet_wrap(~levels) +
    geom_point(aes(color=HCwithin_label),size=5) +
    scale_color_manual(values=c(pal1090[1],pal1090[2]),labels=c("-0.5 std HC","+0.5 std HC")) + 
  ...
  fname = paste(behavmodel,'-',totest,"_",toalign, "_emmeans_", toprocess, "_", 'Value-by-HC2','-',hc_LorR, ".pdf", sep = "")
  pdf(fname, width = 9, height = 3.5)
  gg1 <- ggplot(emt,aes(x=evt_time,y=estimate)) + 
    facet_wrap(~levels) +
    geom_point(aes(color=HCwithin_label),size=5) +
