...
emm <- emm %>% mutate(entropy = case_when(v_entropy_wi=='-1.5'~'low',
                                          v_entropy_wi=='1.5'~'high'))
...
setwd('/Volumes/Users/Andrew/Fig2_Entropy_vPFC')

pdf('DMN-entropy-MMClock.pdf',width=6,height=3.5)
gg1 <- ggplot(emm, aes(x=evt_time,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=entropy)) + 
  geom_point(size=5) + 
...
emm <- emm %>% filter(v_entropy_wi=='-2' | v_entropy_wi=='2')
emm <- emm %>% mutate(entropy = case_when(v_entropy_wi=='-2'~'low',
                                          v_entropy_wi=='2'~'high'))
...
pdf('DMN-entropy-Explore.pdf',width=6,height=3.5)
gg1 <- ggplot(emm, aes(x=evt_time,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=entropy)) + 
  geom_point(size=5) + 
