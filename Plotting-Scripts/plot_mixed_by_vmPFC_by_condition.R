plot_mixed_by_vmPFC_by_condition <- function(ddf,toalign,toprocess,totest,behavmodel,model_iter){
  
  ## Check plots
  if (strcmp(toalign,"clock")){
    setwd('~/vmPFC/MEDUSA Schaefer Analysis/validate_mixed_by_clock_by_condition')
  } else if (strcmp(toalign,"feedback")){
    setwd('~/vmPFC/MEDUSA Schaefer Analysis/validate_mixed_by_feedback_by_condition')
  ...
  for (fe in terms) {
    ...
    if (strcmp(toprocess,"network")){
      fname = paste('rewFunc','-',behavmodel,'-',totest,"_",toalign, "_line_", toprocess, "_", termstr,'_',model_iter,  ".pdf", sep = "")
