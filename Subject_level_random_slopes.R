...
if (strcmp(toalign,'feedback')){
  ...
  df <- df %>% select(id,run,run_trial,trial_bin,outcome,rewFunc,trial_neg_inv_sc,rt_csv_sc,v_entropy_sc,last_outcome,v_max_wi,trial_neg_inv_sc,rt_csv_sc,v_entropy_wi_change,score_sc,rt_bin,iti_sc,ev_sc,expl_longer,expl_shorter)
  ...
  df <- df %>% select(ev,iti_ideal,score_csv,v_max,outcome,v_entropy,rt_lag,v_entropy_full,v_entropy_wi_full,rt_vmax_full,rt_vmax_change_full,rt_csv_sc,rt_csv,id, run, run_trial, last_outcome, trial_neg_inv_sc,pe_max, rt_vmax, score_csv,
                      v_max_wi, v_entropy_wi,kld4_lag,kld4,rt_change,total_earnings, rewFunc,rt_csv, pe_max,v_chosen,rewFunc,iti_ideal,
                      rt_vmax_lag_sc,rt_vmax_change,outcome,pe_max,kld3_lag,rt_lag_sc,rt_next,v_entropy_wi_change,pe_max_lag) %>% 
    group_by(id, run) %>% 
  ...
  df <- df %>% select(id,run,run_trial,trial_bin,rewFunc,iti_sc,trial_neg_inv_sc,rt_csv_sc,v_entropy_wi,expl_longer,expl_shorter,rt_bin,trial_bin,last_outcome,v_max_wi,v_entropy_wi_change_lag,score_lag_sc,iti_lag_sc,ev_lag_sc)
  Q <- merge(df, Q, by = c("id", "run", "run_trial")) %>% arrange("id","run","run_trial","evt_time")
  Q$expl_longer <- relevel(as.factor(Q$expl_longer),ref='0')
  Q$expl_shorter <- relevel(as.factor(Q$expl_shorter),ref='0')
  Q$rt_bin <- relevel(as.factor(Q$rt_bin),ref='-0.5')
  ...
  rm(decode_formula)
  decode_formula <- formula(~ (1|id))
  decode_formula[[1]] = formula(~ age + female + v_entropy_wi*HCwithin + trial_neg_inv_sc + v_max_wi + rt_csv_sc  + iti_sc + iti_lag_sc + last_outcome*HCwithin + outcome + HCbetween*HCbetween + (1 + HCwithin*v_entropy_wi |id) + (1|run))
  decode_formula[[2]] = formula(~ age + female + trial_neg_inv_sc + v_max_wi*HCwithin + rt_csv_sc  + iti_lag_sc + iti_sc + last_outcome*HCwithin + outcome + HCbetween*HCwithin + (1 + HCwithin*v_max_wi  |id) + (1|run))
}
...
for (i in 1:2){
  ...
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  if (j ==1){
    save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'-pred-rt_csv_sc-rt_lag-',i,'.Rdata'))
  } else if(j==2){
    save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'ranslope-pred-rt_csv_sc-rt_lag-',i,'.Rdata'))
  } else if (j==3){
    save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'-pred-rt_csv_sc-rt_vmax_lag-',i,'.Rdata'))
  } else if (j==4){
    save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'ranslope-pred-rt_csv_sc-rt_vmax_lag-',i,'.Rdata'))
  }
  }
}
}
...
if (do_rt_pred_meg) {
  ...
  for (i in 1:2){
    ...
    decode_formula[[2]] = formula(~ rt_lag_sc*subj_level_rand_slope + last_outcome + outcome + v_entropy_wi + v_max_wi +  (1 + rt_lag_sc | id/run))
    decode_formula[[3]] = formula(~ rt_vmax_lag_sc*subj_level_rand_slope + last_outcome + outcome + v_entropy_wi + v_max_wi +  (1 | id/run))  
    decode_formula[[4]] = formula(~ rt_vmax_lag_sc*subj_level_rand_slope + last_outcome + outcome + v_entropy_wi + v_max_wi +  (1 + rt_vmax_lag_sc | id/run))   
    ...
    for (j in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      if (j ==1){
        save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'replication-pred-rt_csv_sc-rt_lag-',i,'.Rdata'))
      } else if(j==2){
        save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'replication-ranslope-pred-rt_csv_sc-rt_lag-',i,'.Rdata'))
      } else if (j==3){
        save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'replication-pred-rt_csv_sc-rt_vmax_lag-',i,'.Rdata'))
      } else if (j==4){
        save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-ranslopes-',toalign,'replication-ranslope-pred-rt_csv_sc-rt_vmax_lag-',i,'.Rdata'))
      }
    }
  }
}
...
load('2023-01-10-vmPFC-HC-network-ranslopes-clock-pred-rt_csv_sc-rt_lag-2.Rdata')
...
load('2023-01-10-vmPFC-HC-network-ranslopes-clockreplication-pred-rt_csv_sc-rt_lag-2.Rdata')
...
ddq <- ddq %>% filter(HC_region=='AH')
ggplot(ddq,aes(x=evt_time,y=rt_lag_sc.trend)) + 
...
load('2023-01-10-vmPFC-HC-network-ranslopes-clockreplication-pred-rt_csv_sc-rt_vmax_lag-2.Rdata')
ddf2 <- ddq$coef_df_reml
...
ddq$p_level_fdr <- factor(ddq$p_level_fdr, levels=c('NS','p < 0.05','p < 0.01','p < 0.001','p < 0.0001'))
ddq <- ddq %>% filter(HC_region=='AH')
ggplot(ddq,aes(x=evt_time,y=rt_vmax_lag_sc.trend)) + 
  facet_grid(network~dataset) +
  geom_line(aes(group=subj_level_rand_slope,color=subj_level_rand_slope)) + 
  geom_point(aes(alpha=p_level_fdr,size=p_level_fdr,color=subj_level_rand_slope)) + 
