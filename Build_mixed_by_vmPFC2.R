...
if (do_vPFC_fb){
  ...
  df <- df %>% select(id,run,run_trial,rt_vmax_change_sc,last_outcome,v_entropy_wi_change,v_entropy_wi_change_lag, trial_bin,iti_ideal,iti_prev,rt_csv,trial_neg_inv_sc,rt_csv_sc,rewFunc,v_entropy_sc,outcome,v_max_wi,score_sc,rt_bin,iti_sc,ev_sc,expl_longer,expl_shorter)
  ...
  decode_formula[[1]] = formula(~ age + female + v_entropy_sc + trial_neg_inv_sc + rt_csv_sc + iti_sc + rt_vmax_change_sc + last_outcome + outcome + (1|id/run))
  ...
  decode_formula[[5]] = formula(~ age + female + trial_neg_inv_sc + v_max_wi + rt_csv_sc + iti_sc + rt_vmax_change_sc + last_outcome + outcome +  (1|id/run))
