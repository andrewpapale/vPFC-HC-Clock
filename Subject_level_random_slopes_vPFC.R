...
if (do_rand_slopes){
  ...
  if (strcmp(toalign,'feedback')){
    ...
    decode_formula[[1]] = formula(~ v_entropy_wi + (1 + v_entropy_wi|id/run))
    decode_formula[[2]] = formula(~ v_max_wi + (1 + v_max_wi |id/run))
    #decode_formula[[3]] = formula(~ age + female + v_entropy_wi + trial_neg_inv_sc + outcome + last_outcome + rt_csv_sc + iti_sc + iti_prev_sc + (1 + v_entropy_wi | rewFunc) + (1|id/rewFunc))
    #decode_formula[[4]] = formula(~ age + female + v_max_wi + trial_neg_inv_sc + outcome + last_outcome + rt_csv_sc + iti_sc + iti_prev_sc + (1 + v_max_wi | rewFunc) + (1 |  id/rewFunc))
  ...
  for (i in 1:2){
    ...
    for (j in 1:length(decode_formula)){
      ...
      if (j==1){
        ...
        save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-pred-rt_csv_sc-both-rs-',i,'.Rdata'))
      } else if (j==2){
  ...
  for (i in 1:2){
    ...
    for (j in 1:length(decode_formula)){
      ...
      if (j==1){
        ...
        save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-replication-',toalign,'-pred-rt_csv_sc-both-rs-',i,'.Rdata'))
      } else if (j==2){
