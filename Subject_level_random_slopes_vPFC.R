...
if (do_rand_slopes){
  ...
  if (strcmp(toalign,'feedback')){
    ...
    decode_formula[[1]] = formula(~ age + female + trial_neg_inv_sc + last_outcome + rt_csv_sc + iti_sc + iti_prev_sc + (1 + v_entropy_wi | id) + (1 | run))
    decode_formula[[2]] = formula(~ age + female + trial_neg_inv_sc + last_outcome + rt_csv_sc + iti_sc + iti_prev_sc + (1 + v_max_wi  |id) + (1 | run))
    ...
    #decode_formula[[3]] = formula(~ age + female + v_entropy_wi + trial_neg_inv_sc + outcome + last_outcome + rt_csv_sc + iti_sc + iti_prev_sc + (1 + v_entropy_wi | rewFunc) + (1|id/rewFunc))
    #decode_formula[[4]] = formula(~ age + female + v_max_wi + trial_neg_inv_sc + outcome + last_outcome + rt_csv_sc + iti_sc + iti_prev_sc + (1 + v_max_wi | rewFunc) + (1 |  id/rewFunc))
    decode_formula[[1]] = formula(~ age + female + last_outcome + rt_lag_sc + iti_lag_sc + (1 + v_entropy_wi |id) + (1|run))
    decode_formula[[2]] = formula(~ age + female + last_outcome + rt_lag_sc + iti_lag_sc + (1 + v_max_wi |id) + (1|run))
    #decode_formula[[3]] = formula(~ age + female + v_max_wi + v_entropy_wi + last_outcome + rt_lag_sc + iti_lag_sc + (1 + v_entropy_wi + v_max_wi |id) + (1|run))
    #decode_formula[[3]] = formula(~ age + female + v_max_wi + v_entropy_wi + trial_neg_inv_sc + last_outcome + rt_lag_sc + iti_lag_sc + (1 + v_max_wi + v_entropy_wi  |id) + (1 | run))
  ...
  for (i in 1:length(decode_formula)){
    ...
    save(ddf,file=paste0(curr_date,'-vmPFC-network-',toalign,'-ranslopes-nofixedeffect-',i,'.Rdata'))
  }
}
...
if (do_rt_pred_fmri){
  for (i in 1:2){
    setwd('~/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection/')
    model_str <- paste0('-vmPFC-network-',toalign,'-ranslopes-nofixedeffect-',i,'.Rdata')
    ...
    df <- df %>% select(rewFunc,rt_vmax_lag,rt_lag,ev,score_csv,v_max,last_outcome,v_entropy,rt_lag,v_entropy_full,v_entropy_wi_full,rt_vmax_full,rt_vmax_change_full,rt_csv_sc,rt_csv,id, run, run_trial, last_outcome, trial_neg_inv_sc,pe_max, rt_vmax, score_csv,
                        ...
                        run_trial,trial_neg_inv_sc,rt_vmax_lag_sc,rt_vmax_change,outcome,pe_max,kld3_lag,rt_lag_sc,rt_next,v_entropy_wi_change,pe_max_lag) %>% 
      group_by(id, run) %>% 
      mutate(rt_sec = rt_csv/1000) %>% 
      mutate(v_chosen_sc = scale(v_chosen),
             run_trial_sc = scale(run_trial),
    ...
    if (!simple_model && !mod_trial_model){
      ...
      decode_formula[[1]] <- formula(~(run_trial_sc + rt_lag_sc + v_max_wi_lag + v_entropy_wi + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * run_trial_sc * subj_level_rand_slope + (1 | id/run))
      #decode_formula[[2]] <- formula(~(trial_neg_inv_sc + rt_lag_sc + v_max_wi_lag + v_entropy_wi + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * trial_neg_inv_sc * subj_level_rand_slope + (1 + rt_vmax_lag_sc + rt_lag_sc | id/run))
    }     
    ...
    for (j in 1:length(decode_formula)){
      ...
      if (!simple_model && !mod_trial_model){
        ...
        if (j==1){
          ...
          am_lim <- anova(m_lim)
          save(am_dmn,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-dmn-anova-',i,'.Rdata'))
          save(am_ctr,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-ctr-anova-',i,'.Rdata'))
          save(am_lim,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-lim-anova-',i,'.Rdata'))
          save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-',i,'.Rdata'))
        } else if (j==2){
          save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-pred-rt_csv_sc-slo-notimesplit-nofixedeffect-rtvmax_by_trial-',i,'.Rdata'))
        }
      } else if (simple_model){
        ddq <- mixed_by(Q2, outcomes = "rt_csv_sc", rhs_model_formulae = decode_formula[[j]], split_on = splits,return_models=TRUE,
                        padjust_by = "term", padjust_method = "fdr", ncores = ncores, refit_on_nonconvergence = 3,
...
if (do_rt_pred_meg){
  for (i in 1:2){
    ...
    df <- df %>% select(rewFunc,rt_vmax_lag,rt_lag,ev,score_csv,v_max,last_outcome,v_entropy,rt_lag,v_entropy_full,v_entropy_wi_full,rt_vmax_full,rt_vmax_change_full,rt_csv_sc,rt_csv,id, run, run_trial, last_outcome, trial_neg_inv_sc,pe_max, rt_vmax, score_csv,
                        run_trial,v_max_wi,v_entropy_wi,kld4_lag,kld4,rt_change,total_earnings, rewFunc,rt_csv, pe_max,v_chosen,rewFunc,
    ...
    for (j in 1:length(decode_formula)){
      ...
      if (!simple_model && !mod_trial_model){
        ...
        if (j==1){
          ...
          save(am_lim,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-replication-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-lim-anova-',i,'.Rdata'))
          save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-replication-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-',i,'.Rdata'))
        } else if (j==2){
          save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-replication-pred-rt_csv_sc-slo-notimesplit-nofixedeffect-',i,'.Rdata'))
        }
      } else if (simple_model){
        ddq <- mixed_by(Q2, outcomes = "rt_csv_sc", rhs_model_formulae = decode_formula[[j]], split_on = splits,return_models=TRUE,
                        padjust_by = "term", padjust_method = "fdr", ncores = ncores, refit_on_nonconvergence = 3,
