...
for (i in 1){
  ...
  save(ddf,file=paste0(curr_date,'-vmPFC-symmetry-feedback-split-by-condition-',i,'.Rdata'))
}
...
for (i in 1){
  ...
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-network-feedback-split-by-condition-',i,'.Rdata'))
}
...
df <- df %>% select(id,run,trial_bin,last_outcome,rewFunc,rt_bin,v_max_wi,expl_longer,expl_shorter,rt_csv_sc,v_entropy_wi, v_entropy_wi_change,run_trial,trial_neg_inv_sc,rt_vmax_change,kld3,abs_pe_max_sc,abs_pe_max_lag_sc,pe_max,pe_max_lag,pe_max_sc,pe_max_lag_sc,v_entropy_wi_change_lag)
Q <- merge(df, vmPFC, by = c("id", "run", "run_trial")) %>% arrange("id","run","run_trial","evt_time")
Q$expl_longer <- relevel(as.factor(Q$expl_longer),ref='0')
Q$expl_shorter <- relevel(as.factor(Q$expl_shorter),ref='0')
Q$rt_bin <- relevel(as.factor(Q$rt_bin),ref='-0.5')
...
#                                expl_shorter + expl_longer +  # binary expl_code incr / decr separate variables
#                               (1|id))
decode_formula[[1]] <- formula(~ v_max_wi + trial_neg_inv_sc + v_entropy_wi + rt_bin + expl_shorter + expl_longer + trial_neg_inv_sc*v_entropy_wi + (1|id))
splits = c('evt_time','symmetry_group','rewFunc')
source("~/fmri.pipeline/R/mixed_by.R")
for (i in 1){
  setwd('~/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
  ...
  ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = df0 , split_on = splits,
                  ...
                  tidy_args = list(effects=c("fixed","ran_vals","ran_pars","ran_coefs"),conf.int=TRUE))
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-symmetry-clock-split-by-condition-',i,'.Rdata'))
}
...
for (i in 1){
  ...
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-network-clock-split-by-condition-trial_x_entropy-',i,'.Rdata'))
}
...
for (i in 1){
  ...
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-feedback-split-by-condition-',i,'.Rdata'))
}
...
for (i in 1){
  ...
  ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = df0 , split_on = splits,
                  ...
                  tidy_args = list(effects=c("fixed","ran_vals","ran_pars","ran_coefs"),conf.int=TRUE))
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-HC-symmetry-feedback-split-by-condition-',i,'.Rdata'))
}
###################################
##### vmPFC - HC - clock      #####
###################################
...
for (i in 1){
  ...
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-clock-split-by-condition-',i,'.Rdata'))
}
...
for (i in 1){
  ...
  curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
  save(ddf,file=paste0(curr_date,'-vmPFC-HC-symmetry-clock-split-by-condition-',i,'.Rdata'))
}