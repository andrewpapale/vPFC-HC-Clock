...
if (do_rt_pred_fmri){
  for (i in 1:3){
    ...
    if (!simple_model && !trial_mod){
      ...
      decode_formula[[1]] <- formula(~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run))
      ...
      if (!simple_model && !trial_mod){
        ...
        ddq <- mixed_by(Q2, outcomes = "rt_csv_sc", rhs_model_formulae = decode_formula[[j]], split_on = splits,return_models=TRUE,
                        ...
                        emtrends_spec = list(
                          ...
                          TrxVmax1 = list(outcome='rt_csv_sc',model_name='model1', var = 'run_trial0_neg_inv_sc',
        ...
        if (j==1){
          # AH
          zq <- Q2 %>% filter(network=='DMN' & HC_network =='AH')
          m_dmn_ah <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_dmn_ah <- anova(m_dmn_ah)
          zq <- Q2 %>% filter(network=='CTR' & HC_network =='AH')
          m_ctr_ah <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_ctr_ah <- anova(m_ctr_ah)
          zq <- Q2 %>% filter(network=='LIM' & HC_network =='AH')
          m_lim_ah <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_lim_ah <- anova(m_lim_ah)
          # PH
          zq <- Q2 %>% filter(network=='DMN' & HC_network =='PH')
          m_dmn_ph <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_dmn_ph <- anova(m_dmn_ph)
          zq <- Q2 %>% filter(network=='CTR' & HC_network =='PH')
          m_ctr_ph <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_ctr_ph <- anova(m_ctr_ph)
          zq <- Q2 %>% filter(network=='LIM' & HC_network =='PH')
          m_lim_ph <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_lim_ph <- anova(m_lim_ph)
          save(am_dmn_ah,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-dmn_ah-anova-',i,'.Rdata'))
          save(am_ctr_ah,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-ctr_ah-anova-',i,'.Rdata'))
          save(am_lim_ah,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-lim_ah-anova-',i,'.Rdata'))
          save(am_dmn_ph,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-dmn_ph-anova-',i,'.Rdata'))
          save(am_ctr_ph,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-ctr_ph-anova-',i,'.Rdata'))
          save(am_lim_ph,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-lim_ph-anova-',i,'.Rdata'))
          save(ddq,file=paste0(curr_date,'-vmPFC-HC-network-Explore-ranslopes-',toalign,'-pred-int-notimesplit-nofixedeffect-rtvmax_by_trial-',i,'.Rdata'))
        } else {
