...
df <- df %>% group_by(id,run) %>% mutate(entropy_split = case_when(
  v_entropy < mean(v_entropy,na.rm=TRUE) ~ 'low',
  v_entropy > mean(v_entropy,na.rm=TRUE) ~ 'high'
)) %>% ungroup()
df <- df %>% select(outcome,entropy_split,v_max_above_median,id,run,trial_bin,rewFunc,rt_bin,v_max_wi,expl_longer,expl_shorter,rt_csv_sc,v_entropy_wi, v_entropy_wi_change,run_trial,trial_neg_inv_sc,rt_vmax_change,kld3,abs_pe_max_sc,abs_pe_max_lag_sc,pe_max,pe_max_lag,pe_max_sc,pe_max_lag_sc,v_entropy_wi_change_lag)
...
dQ <- Q %>% group_by(id,evt_time,network,entropy_split) %>% summarize(vP = mean(vmPFC_decon,na.rm=TRUE),dP = sd(vmPFC_decon,na.rm=TRUE), N = length(vmPFC_decon))
dQ <- dQ %>% ungroup() %>% group_by(evt_time,network,entropy_split) %>% summarize(vP1 = mean(vP,na.rm=TRUE),dP1 = sd(dP,na.rm=TRUE), N1 = length(N))
...
rm(dQ)
dQ <- Q %>% group_by(id,evt_time,network,v_max_above_median) %>% summarize(vP = mean(vmPFC_decon,na.rm=TRUE),dP = sd(vmPFC_decon,na.rm=TRUE), N = length(vmPFC_decon))
dQ <- dQ %>% ungroup() %>% group_by(evt_time,network,v_max_above_median) %>% summarize(vP1 = mean(vP,na.rm=TRUE),dP1 = sd(dP,na.rm=TRUE), N1 = length(N))
dQ <- dQ %>% mutate(network1 = case_when(network=='D'~'DMN',network=='C'~'CTR',network=='L'~'LIM'))
dQ$network1 <- factor(dQ$network1,levels=c('DMN','CTR','LIM'))
...
df <- df %>% group_by(id,run) %>% mutate(v_entropy_lag = lag(v_entropy))
df <- df %>% group_by(id,run) %>% mutate(entropy_split = case_when(
  v_entropy_lag < mean(v_entropy,na.rm=TRUE) ~ 'low',
  v_entropy_lag > mean(v_entropy,na.rm=TRUE) ~ 'high'
)) %>% ungroup()
df <- df %>% group_by(id,run) %>% mutate(v_max_wi_lag = lag(v_max_wi)) %>% ungroup()
df <- df %>% group_by(id,run) %>% mutate(v_max_above_median=case_when(
  v_max_wi_lag < median(v_max_wi_lag,na.rm=TRUE) ~ FALSE,
  v_max_wi_lag > median(v_max_wi_lag,na.rm=TRUE) ~ TRUE
)) %>% ungroup()
df <- df %>% select(last_outcome,id,run,trial_bin,rewFunc,rt_bin,entropy_split,v_max_above_median,v_max_wi,expl_longer,expl_shorter,rt_csv_sc,v_entropy_wi, v_entropy_wi_change,run_trial,trial_neg_inv_sc,rt_vmax_change,kld3,abs_pe_max_sc,abs_pe_max_lag_sc,pe_max,pe_max_lag,pe_max_sc,pe_max_lag_sc,v_entropy_wi_change_lag)
Q <- merge(df, vmPFC, by = c("id", "run", "run_trial")) %>% arrange("id","run","run_trial","evt_time")
Q$expl_longer <- relevel(as.factor(Q$expl_longer),ref='0')
Q$expl_shorter <- relevel(as.factor(Q$expl_shorter),ref='0')
Q$rt_bin <- relevel(as.factor(Q$rt_bin),ref='-0.5')
