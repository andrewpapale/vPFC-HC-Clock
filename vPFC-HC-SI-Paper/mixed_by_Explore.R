...
demo <- readRDS('/Volumes/Users/Andrew/MEDuSA_data_Explore/explore_complete_demographics.rds')
...
if (do_vPFC_HC){
  ...
  demo <- readRDS('/Volumes/Users/Andrew/MEDuSA_data_Explore/explore_complete_demographics.rds')
  demo <- demo %>% select(id,age,sex,Group_d,wtar,education_yrs,max_lethality,max_intent)
  demo <- demo %>% rename(gender = sex)
  demo$gender <- relevel(as.factor(demo$gender),ref='M')
  demo$age <- scale(demo$age)
  demo$wtar <- scale(demo$wtar)
  demo$education_yrs <- scale(demo$education_yrs)
  ...
  Q$Group_d <- relevel(factor(Q$Group_d),ref='Controls')
  #Q <- Q %>% filter(group!='ATT')
  #Q <- Q %>% filter(group=='HC')
  Q <- Q %>% filter(!is.na(rewFunc))
  #Q <- Q %>% filter(trial > 10)
  ...
  excludes = c(207224, 210374, 210701, 213708, 216806, 216845, 219392, 220024, 221698, 440149, 440311, 440336) #imaging QC excludes
  
  Q <- Q %>% filter(!id %in% excludes)
  ...
  #decode_formula[[1]] = formula(~age*HCwithin + gender*HCwithin + run_trial0_neg_inv_sc*HCwithin + rt_lag_sc*HCwithin + last_outcome*HCwithin + iti_lag_sc*HCwithin + scale(max_lethality)*HCwithin + HCbetween + (1|id/run))
  #decode_formula[[2]] = formula(~age + gender + run_trial0_neg_inv_sc + rt_lag_sc + last_outcome + v_entropy_wi + v_max_wi + iti_lag_sc + HCwithin*scale(max_lethality) + (1|id/run))
  #decode_formula[[3]] = formula(~age + gender + last_outcome + HCwithin*scale(max_lethality) + (1|id/run))
  #decode_formula[[4]] = formula(~age + gender + HCwithin*group + (1|id/run))
  decode_formula[[1]] = formula(~age*HCwithin + gender*HCwithin + v_max_wi*HCwithin*Group_d + run_trial0_neg_inv_sc*HCwithin + rt_lag_sc*HCwithin + last_outcome*HCwithin + iti_lag_sc*HCwithin + HCbetween + (1|id/run))
  decode_formula[[3]] = formula(~age*HCwithin + gender*HCwithin + v_entropy_wi*HCwithin*Group_d + run_trial0_neg_inv_sc*HCwithin + rt_lag_sc*HCwithin + last_outcome*HCwithin + iti_lag_sc*HCwithin + HCbetween + (1|id/run))
  decode_formula[[2]] = formula(~age*HCwithin + gender*HCwithin + v_max_wi*HCwithin*Group_d + run_trial0_neg_inv_sc + rt_lag_sc + last_outcome + iti_lag_sc + HCbetween + (1|id/run))
  decode_formula[[4]] = formula(~age*HCwithin + gender*HCwithin + v_entropy_wi*HCwithin*Group_d + run_trial0_neg_inv_sc + rt_lag_sc + last_outcome + iti_lag_sc + HCbetween + (1|id/run))
  qT <- c(-0.8,0.46)
  ...
  for (i in 1:length(decode_formula)){
    ...
    if (i < 3){
      ddf <- mixed_by(Q, outcomes = "vmPFC_decon", rhs_model_formulae = df0 , split_on = splits,
                      ...
                      # )#,
                      emtrends_spec = list(
                        GxHC = list(outcome = 'vmPFC_decon',model_name='model1',var = 'v_max_wi',
                                      specs = formula(~v_max_wi:HCwithin:Group_d), at = list(HCwithin = c(-1.5,1.5)))
      ...
      # )#,
      emtrends_spec = list(
        GxHC = list(outcome = 'vmPFC_decon',model_name='model1',var = 'v_entropy_wi',
                      specs = formula(~v_entropy_wi:HCwithin:Group_d), at = list(HCwithin = c(-1.5,1.5)))
        #   HxW = list(outcome='vmPFC_decon',model_name='model1', var = 'v_entropy_wi',
        #              specs = formula(~v_entropy_wi:wtar),at=list(wtar = c(-1,-0.5,0,0.5,1))),
        #   HxA = list(outcome='vmPFC_decon',model_name='model1', var = 'v_entropy_wi',
        #             specs = formula(~v_entropy_wi:age),at=list(age = c(-1,-0.5,0,0.5,1))),
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Explore-vPFC-HC-network-clock-All-SI-paper-',i,'.Rdata'))
  }
