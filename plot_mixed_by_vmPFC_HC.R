plot_mixed_by_vmPFC_HC <- function(ddf,toalign,toprocess,totest,behavmodel,model_iter,hc_LorR,CTRflag,flipy){
  ...
  if (strcmp(toprocess,'network-by-HC') | strcmp(toprocess,'network-by-HC-by-side') | strcmp(toprocess,'network-by-HC-by-scanner') | strcmp(toprocess,'network-by-HC-by-rewFunc') | strcmp(toprocess,'network-by-HC-by-side') | strcmp(toprocess,'network-by-HC-by-sex')){
  ...
  if (!strcmp(totest,'anatomy')){
    for (fe in terms) {
            ...
            if (all(edf$`p, FDR-corrected`=='p < .001')){
              if (fe=='HCwithin'){
                ...
                gg1<-ggplot(edf, aes(x=t, y=estimate,group=network2,color=network2)) + 
                  ...
                  #geom_text(aes(x=-.5, y = .485, label = "RT(Vmax)"), angle = 90, color = "white", size = 2) +
                  theme_bw(base_size=13) +  
                  facet_wrap(side~HC_region) + ylab('Response [AU]') +
                  theme(legend.title = element_blank(),
                        axis.title.y = element_text(margin=margin(r=6),size=22),
                        axis.title.x = element_text(margin=margin(t=6),size=22),
                        legend.text = element_text(size=22),
            ...
            if (all(edf$`p, FDR-corrected`=='p < .001')){
              if (fe=='HCwithin'){
                gg1<-ggplot(edf, aes(x=t, y=estimate,group=network2,color=network2)) + 
                  ...
                  #geom_text(aes(x=-.5, y = .485, label = "RT(Vmax)"), angle = 90, color = "white", size = 2) +
                  theme_bw(base_size=13) + ylab('Response [AU]') + 
                  facet_wrap(scan_which~HC_region) + 
                  theme(legend.title = element_blank(),
                        axis.title.y = element_text(margin=margin(r=6),size=22),
                        axis.title.x = element_text(margin=margin(t=6),size=22),
                        legend.text = element_text(size=22),
                ...
                gg1<-ggplot(edf, aes(x=t, y=estimate,group=network2,color=network2)) + 
                  ...
                  #geom_text(aes(x=-.5, y = .485, label = "RT(Vmax)"), angle = 90, color = "white", size = 2) +
                  theme_bw(base_size=13) +  
                  facet_wrap(scan_which~HC_region) + ylab('Response [AU]') +
                  theme(legend.title = element_blank(),
                        axis.title.y = element_text(margin=margin(r=6),size=22),
                        axis.title.x = element_text(margin=margin(t=6),size=22),
                        legend.text = element_text(size=22),
      ...
      } else if (strcmp(toprocess,'network-by-HC-by-sex')){
            ...
            if (all(edf$`p, FDR-corrected`=='p < .001')){
              if (fe=='HCwithin'){
                ...
                gg1<-ggplot(edf, aes(x=t, y=estimate,group=network2,color=network2)) + 
                  ...
                  #geom_text(aes(x=-.5, y = .485, label = "RT(Vmax)"), angle = 90, color = "white", size = 2) +
                  theme_bw(base_size=13) +  
                  facet_wrap(female~HC_region) + ylab('Response [AU]') +
                  theme(legend.title = element_blank(),
                        axis.title.y = element_text(margin=margin(r=6),size=22),
                        axis.title.x = element_text(margin=margin(t=6),size=22),
                        legend.text = element_text(size=22),
