...
rm(dq3)
dq3 <- df %>% select(rt_swing,rt_csv,rewFunc,run_trial,v_entropy_wi,last_outcome,outcome,rt_vmax_lag,id,run,entropy_split)
dq3 <- dq3 %>% group_by(id,run) %>% mutate(trial_bin = (case_when(
  run_trial <= 15 ~ 'Early',
  run_trial > 15 & run_trial < 30 ~ 'Middle',
  run_trial >=30 ~ 'Late',
)))
dq3 <- dq3 %>% mutate(entropy_bin = ntile(v_entropy_wi,50))
dq3 <- dq3 %>% group_by(rewFunc,entropy_split,run_trial,last_outcome) %>% summarize(mRT = mean(rt_swing,na.rm=TRUE), dRT = sd(rt_swing,na.rm=TRUE),N = length(rt_swing)) %>% ungroup()
...
dq3 <- dq3 %>% filter(!is.na(entropy_split) & !is.na(la))
...
gg1 <- ggplot(dq3, aes(x=run_trial,y=mRT,color=Contingency,group=Contingency)) + 
  ...
  geom_smooth(method='loess',span=1) + ylab('Change in RT (s)') + xlab('Trial') + facet_wrap(last_outcome~entropy_split) +
  theme(axis.text=element_text(size=20),axis.title=element_text(size=40,margin=margin(t=0,r=0,b=10,l=10)), legend.text=element_text(size=20, margin=margin(t=0, r=10, b=8, l=0)), legend.title=element_text(size=40))
print(gg1)
dev.off()
