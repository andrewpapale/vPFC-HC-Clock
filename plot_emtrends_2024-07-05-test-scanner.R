...
if (do_vPFC_HC_entropy){
  ...
  load('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection/2025-01-27-vmPFC-HC-network-Explore-ranslopes-clock-pred-int-trial1-10included-scanner-1.Rdata')
  emt_explore <- ddq$emtrends_list
  ...
  RTxO_explore <- inner_join(RTxO_explore,explore,by=c('network','HC_region','scan_which'))
  

  PH_merged <- RTxO_explore %>% filter(HC_region == 'PH')
  

  AH_merged <- RTxO_explore %>% filter(HC_region == 'AH')
  ...
  AH_summary <- AH_merged %>% 
    group_by(scan_which,network,last_outcome,entropy) %>% 
  ...
  pdf('RT-Pred-Entropy-AH-emtrends-scanner.pdf',height=8,width=10)
  gg1 <- ggplot(AH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) + 
    geom_point(size=8, position=position_dodge(width=0.5), aes(shape = entropy)) + 
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') + 
    facet_grid(last_outcome~scan_which,labeller = label_wrap_gen(width=16), scales='free_y') + scale_color_manual(values = pal) + 
    scale_shape_manual(values = c(1,16)) +
    ylab('<- less - Exploration - more ->') + scale_y_reverse() +
    theme_bw(base_size=13) +
    xlab('Network') +
  ...
  pdf('RT-Pred-Entropy-PH-emtrends-scanner.pdf',height=8,width=10)
  ...
  PH_merged <- RT_explore %>% filter(HC_region == 'PH')
  
  AH_merged <- RT_explore %>% filter(HC_region == 'AH')
  ...
  AH_summary <- AH_merged %>%
    group_by(scan_which,network,entropy) %>%
    summarize(mean_trend = mean(rt_lag_sc.trend,na.rm=TRUE), mean_sd = mean(std.error,na.rm=TRUE), sum_p_ls_001 = sum(p_level_fdr =='4')) %>%
    ungroup()
  ...
  pdf('RT-2way-Pred-Entropy-AH-emtrends-scanner.pdf',height=8,width=10)
  gg1<- ggplot(AH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) +
    geom_point(size=5, position=position_dodge(width=0.5), aes(shape = entropy)) +
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') +
    facet_grid(~scan_which,labeller = label_wrap_gen(width=16)) + scale_color_manual(values = pal) +
    scale_shape_manual(values = c(1,16)) +
    ylab('<- less - Exploration - more ->') + scale_y_reverse() +
    theme_bw(base_size=13) +
    ggtitle('Anterior Hippocampus') +
  ...
  pdf('RT-2way-Pred-Entropy-PH-emtrends-scanner.pdf',height=8,width=10)
  gg1 <- ggplot(PH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) +
    geom_point(size=5, position=position_dodge(width=0.5), aes(shape = entropy)) +
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') +
    facet_grid(~scan_which,labeller = label_wrap_gen(width=16)) + scale_color_manual(values = pal) +
  ...
  PH_merged <- RTvmax_explore %>% filter(HC_region == 'PH')
  
  AH_merged <- RTvmax_explore %>% filter(HC_region == 'AH')
  ...
  pdf('RTvmax-2way-Pred-Entropy-AH-emtrends-scanner.pdf',height=8,width=10)
  gg1<- ggplot(AH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) +
    geom_point(size=5, position=position_dodge(width=0.5), aes(shape = entropy)) +
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') +
    facet_grid(~scan_which,labeller = label_wrap_gen(width=16)) + scale_color_manual(values = pal) +
  ...
  pdf('RTvmax-2way-Pred-Entropy-PH-emtrends-scanner.pdf',height=8,width=10)
  gg1<- ggplot(PH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) +
    geom_point(size=5, position=position_dodge(width=0.5), aes(shape = entropy)) +
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') +
    facet_grid(~scan_which,labeller = label_wrap_gen(width=16)) + scale_color_manual(values = pal) +
  ...
  AH_summary <- AH_merged %>%
    group_by(scan_which,network,entropy,trial_neg_inv_sc) %>%
    summarize(mean_trend = mean(rt_vmax_lag_sc.trend,na.rm=TRUE), mean_sd = mean(std.error,na.rm=TRUE), sum_p_ls_001 = sum(p_level_fdr =='4')) %>%
    ungroup()
  ...
  pdf('RTvmax-3way-Pred-Entropy-AH-emtrends-scanner.pdf',height=8,width=10)
  gg1<- ggplot(AH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) +
    geom_point(size=5, position=position_dodge(width=0.5), aes(shape = entropy)) +
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') +
    facet_grid(trial_neg_inv_sc~scan_which,labeller = label_wrap_gen(width=16)) + scale_color_manual(values = pal) +
    scale_shape_manual(values = c(1,16)) +
    ylab('<- less - Exploitation - more ->') +
    theme_bw(base_size=13) +
    ggtitle('Anterior Hippocampus') +
  ...
  pdf('RTvmax-3way-Pred-Entropy-PH-emtrends-scanner.pdf',height=8,width=10)
  gg1<- ggplot(PH_summary, aes(x=network,y=mean_trend,ymin = mean_trend-mean_sd,ymax=mean_trend+mean_sd,color=network, group=entropy)) +
    geom_point(size=5, position=position_dodge(width=0.5), aes(shape = entropy)) +
    geom_errorbar(width=0.1, position=position_dodge(width=0.5),color = 'black') +
    facet_grid(trial_neg_inv_sc~scan_which,labeller = label_wrap_gen(width=16)) + scale_color_manual(values = pal) +
