...
labels_df <- read_csv("/Volumes/Users/Andrew/PFC_region_gradients.csv")
labels_df <- labels_df %>% select(region, symmetry_group, atlas_value, network)
...
trial_df <- setDT(get_trial_data(repo_directory = "~/clock_analysis", dataset = study)) %>%
# trial_df <- trial_df %>%
  group_by(id, run) %>%
...
#                                              rhs_model_formulae = fmri.pipeline:::named_list(int, slo), ncores = 26, afni_dir = '/Users/dnplserv/abin',
#                                              calculate = c('parameter_estimates_reml', 'fit_statistics'),
#                                              trial_join_col = c('id', 'run_number'), beta_level = 1L, focal_contrast = '{l1_contrasts}', emtrends_spec = emtrends_spec,
...
labels_df <- labels_df %>% rename(mask_value = atlas_value)
to_plot <- mixed_by_betas(beta_csv = efiles_l1, labels_df, trial_df, mask_file = 'Schaefer2018_200Parcels_7Networks_order_fonov_1mm_ants.nii.gz',
                          rhs_model_formulae = fmri.pipeline:::named_list(int, slo), ncores = 15, afni_dir = '/Users/dnplserv/abin',
                          calculate = c('parameter_estimates_reml', 'fit_statistics'),
                          trial_join_col = c('id'), beta_level = 2L, focal_contrast = l1_contrasts, emtrends_spec = emtrends_spec,
                          split_on = c('l1_cope_name', 'mask_value'))
