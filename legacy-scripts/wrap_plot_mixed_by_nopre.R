...
if (do_vPFC_fb){
  if (do_network){
    ...
    for (i in 1:2){
      ...
      model_str <- paste0('-vmPFC-network-feedback-nopre-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
      totest <- 'final-model-nopre-'
      toprocess <- 'network'
      toalign <- 'feedback'
  ...
  if (do_symmetry){
    ...
    for (i in 1:2){
      setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
      model_str <- paste0('-vmPFC-symmetry-feedback-nopre-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
      totest <- 'final-model-nopre-'
...
if (do_vPFC_clock){
  if (do_network){
    ...
    for (i in 1:2){
      setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
      model_str <- paste0('-vmPFC-network-clock-nopre-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
      totest <- 'final-model-nopre-'
  ...
  if (do_symmetry){
    ...
    for (i in 1:2){
      setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
      model_str <- paste0('-vmPFC-symmetry-clock-nopre-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
      totest <- 'final-model-nopre-'
...
if (do_HC_fb){
  ...
  for (i in 1){
    setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
    model_str <- paste0('-HC-axis-feedback-nopre-',i,'.Rdata')
    model_str <- Sys.glob(paste0('*',model_str))
    load(model_str)
    model_iter <- i
    totest <- 'final-model-nopre-'
...
if (do_HC_clock){
  ...
  for (i in 1:2){
    setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
    model_str <- paste0('-HC-axis-clock-nopre-',i,'.Rdata')
    model_str <- Sys.glob(paste0('*',model_str))
    load(model_str)
    model_iter <- i
    totest <- 'final-model-nopre-'
...
if (do_HC2vPFC_fb){
  if (do_network){
    ...
    for (i in 1:2){
      ...
      model_str <- paste0('-vmPFC-HC-network-feedback-nopre-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
      totest <- 'final-model-nopre-'
  ...
  if (do_symmetry){
    ...
    for (i in 1:2){
      setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
      model_str <- paste0('-vmPFC-HC-symmetry-feedback-nopre-',i,'.Rdata')
...
if (do_HC2vPFC_clock){
  if (do_network){
    ...
    for (i in 1:2){
      setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
      #model_str <- paste0('-vmPFC-HC-full-symmetry-',i,'.Rdata')
      model_str <- paste0('vmPFC-HC-network-clock-nopre-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
      totest <- 'final-model-nopre-'
  ...
  if (do_symmetry){
    ...
    for (i in 1:2){
      setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
      model_str <- paste0('-vmPFC-HC-symmetry-clock-nopre-',i,'.Rdata')
      #model_str <- paste0('-vmPFC-HC-full-network-',i,'.Rdata')
      model_str <- Sys.glob(paste0('*',model_str))
      load(model_str)
      model_iter <- i
...
# nice HCwithin plot
setwd('/Users/dnplserv/vmPFC/MEDUSA Schaefer Analysis/vmPFC_HC_model_selection')
model_str <- paste0('-vmPFC-HC-network-clock-nopre-',1,'.Rdata')
model_str <- Sys.glob(paste0('*',model_str))
load(model_str)
...
pdf('Nice-HC-vPFC-Value-emt.pdf',height=4,width=12)
