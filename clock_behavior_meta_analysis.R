...
df <- df %>% select(!run) %>% rename(run = scanner_run) 
...
df <- df %>% select(dataset,id,alpha_ffx,beta_ffx,gamma_ffx,age,sex,rt_lag_sc,v_entropy_wi,rt_csv_sc,rt_vmax_lag_sc,trial_neg_inv_sc,run,last_outcome,v_max_wi,total_earnings,v_max,v_entropy,ev,magnitude,probability)
dfmmc <- dfmmc %>% select(dataset,id,alpha_ffx,beta_ffx,gamma_ffx,age,sex,rt_lag_sc,v_entropy_wi,rt_csv_sc,rt_vmax_lag_sc,trial_neg_inv_sc,run,last_outcome,v_max_wi,total_earnings,v_max,v_entropy,ev,magnitude,probability)
...
dfmmc_meg <- dfmmc_meg %>% select(dataset,id,alpha_ffx,beta_ffx,gamma_ffx,age,sex,rt_lag_sc,v_entropy_wi,rt_csv_sc,rt_vmax_lag_sc,trial_neg_inv_sc,run,last_outcome,v_max_wi,total_earnings,v_max,v_entropy,ev,magnitude,probability)
...
dfmer0 <- dfmer1 %>% group_by(id,dataset) %>% summarize(med_vmax = median(v_max,na.rm=TRUE), med_ventropy = median(v_entropy,na.rm=TRUE), med_mag = median(magnitude,na.rm=TRUE), med_prob = median(probability,na.rm=TRUE), med_ev = median(ev,na.rm=TRUE),alpha_ffx = alpha_ffx[1],beta_ffx=beta_ffx[1],gamma_ffx=gamma_ffx[1],total_earnings=total_earnings[1],age = age[1],sex=sex[1],dataset=dataset[1]) %>% ungroup()
...
m_mmc_fmri_tE <- lm(data = dfmer0 %>% filter(dataset=='MMClock fMRI'), total_earnings ~ age)
m_mmc_meg_tE <- lm(data = dfmer0 %>% filter(dataset=='MMClock MEG'), total_earnings ~ age)
m_bsoc_fmri_tE <- lm(data = dfmer0 %>% filter(dataset=='Bsocial'), total_earnings ~ age)
...
# 
# decode_formula <- NULL
# decode_formula[[1]] <- formula(~(rt_lag_sc + total_earnings + last_outcome)^2 + rt_lag_sc:last_outcome:total_earnings + rt_vmax_lag_sc * total_earnings * trial_neg_inv_sc + (1 | id/run))
# 
...
#                         TrxVmax = list(outcome='rt_csv_sc',model_name='model1',
#                                        specs=formula(~total_earnings:rt_vmax_lag_sc:trial_neg_inv_sc), at= list(rt_vmax_lag_sc=c(-2,-1,0,1,2),trial_neg_inv_sc=c(-0.9,-0.02,0.2,0.34,0.4)))
#                      
#                       ),
#                      emtrends_spec = list(
#                      #   RT = list(outcome='rt_csv_sc', model_name='model1', var='rt_lag_sc',
...
#                          TrxVmax = list(outcome='rt_csv_sc',model_name='model1', var = 'rt_vmax_lag_sc',
#                                         specs=formula(~total_earnings:rt_vmax_lag_sc:trial_neg_inv_sc), at= list(trial_neg_inv_sc=c(-0.9,-0.02,0.2,0.34,0.4),total_earnings = c(-1,-1,1,2)))#,
#                         # TrxVmax1 = list(outcome='rt_csv_sc',model_name='model1', var = 'trial_neg_inv_sc',
#                         #                 specs=formula(~total_earnings:rt_vmax_lag_sc:trial_neg_inv_sc), at= list(rt_vmax_lag_sc=c(-2,-1,0,1,2),total_earnings = c(-1,-1,0,1,2))),
#                         # TrxVmax2 = list(outcome='rt_csv_sc',model_name='model1', var = 'total_earnings',
#                         #                  specs=formula(~total_earnings:rt_vmax_lag_sc:trial_neg_inv_sc), at= list(rt_vmax_lag_sc=c(-2,-1,0,1,2),trial_neg_inv_sc=c(-0.9,-0.02,0.2,0.34,0.4)))
#                      )
# )
