...
if (do_MMClock){
  ...
  Q <- Q %>% mutate(HC_split = case_when(HCwithin >= 1.5 ~ '>= 1.5 std',
                                         HCwithin > 0 & HCwithin < 1.5 ~ '> 0 & < 1.5 std',
                                         HCwithin <= 0 & HCwithin > -1.5 ~ '<= 0 & > -1.5 std',
                                         HCwithin <= -1.5 ~ '<= -1.5 std'))
  
  Q <- Q %>% filter(!is.na(HC_split))
  ...
  splits = c('online','network','HC_region','HC_split')
  source("~/fmri.pipeline/R/mixed_by.R")
  for (i in 1:length(decode_formula)){
    ...
    save(ddf,file=paste0(curr_date,'-vPFC-HC-network-clock-HC_split-',i,'.Rdata'))
  }
}
...
if (do_plot_MMClock){
  for (iM in 1:3){
    load(paste0(repo_str,'/','2023-05-26-vPFC-HC-network-clock-HC_split-',iM,'.Rdata')) # ddf
    ...
    title_str <- paste0(paper_version,'-MMClock-clock-network-by-HC-by-HC_split-HCwithin-',iM,'.pdf')
    pdf(title_str,height=8,width=16)
    
    if (all(hc_w$p_level_fdr=='p < 0.001')){
      gg1 <- ggplot(hc_w,aes(x=HC_split,y=estimate)) + 
        ...
        theme(legend.title = element_blank(),
              ...
              axis.text.x = element_text(angle = 90, vjust=0.5,hjust=1,size=22),
    ...
    if (nrow(hc_en)>0){
      title_str <- paste0(paper_version,'-MMClock-clock-network-by-HC-by-HC_split-HCwithin-v_entropy_wi-',iM,'.pdf')
      pdf(title_str,height=8,width=16)
      
      if (all(hc_en$p_level_fdr=='p < 0.001')){
        gg1 <- ggplot(hc_en,aes(x=HC_split,y=estimate)) + 
          geom_line(position=position_dodge(width=0.2),aes(color=network,group=network)) + 
          geom_point(position=position_dodge(width=0.2),aes(color=network,group=network,size=p_level_fdr,alpha=p_level_fdr)) + 
          geom_errorbar(position=position_dodge(width=0.2),width=0,color='white',aes(ymin=estimate-std.error,ymax=estimate+std.error,group=network)) +
          facet_grid(online~HC_region) + 
    ...
    if (nrow(hc_vmax)>0){
      title_str <- paste0(paper_version,'-MMClock-clock-network-by-HC-by-HC_split-HCwithin-v_max_wi-',iM,'.pdf')
      pdf(title_str,height=8,width=16)
      
      if (all(hc_vmax$p_level_fdr=='p < 0.001')){
        gg1 <- ggplot(hc_vmax,aes(x=HC_split,y=estimate)) + 
          geom_line(position=position_dodge(width=0.2),aes(color=network,group=network)) + 
          geom_point(position=position_dodge(width=0.2),aes(color=network,group=network,size=p_level_fdr,alpha=p_level_fdr)) + 
          geom_errorbar(position=position_dodge(width=0.2),width=0,color='white',aes(ymin=estimate-std.error,ymax=estimate+std.error,group=network)) +
          facet_grid(online~HC_region) + 
...
if (do_Explore){
  ...
  for (i in 1:length(decode_formula)){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-vPFC-HC-Explore-network-clock-HC_split-',i,'.Rdata'))
  }
}
...
if (do_plot_Explore){
  for (iM in 1:3){
    ...
    title_str <- paste0(paper_version,'-Explore-clock-network-by-HC-by-HC_split-HCwithin-',iM,'.pdf')
    pdf(title_str,height=8,width=16)
    ...
    if (nrow(hc_en)>0){
      title_str <- paste0(paper_version,'-Explore-clock-network-by-HC-by-HC_split-HCwithin-v_entropy_wi-',iM,'.pdf')
      pdf(title_str,height=8,width=16)
    ...
    if (nrow(hc_vmax)>0){
      title_str <- paste0(paper_version,'-Explore-clock-network-by-HC-by-HC_split-HCwithin-v_max_wi-',iM,'.pdf')
      pdf(title_str,height=8,width=16)
