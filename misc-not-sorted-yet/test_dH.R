...
if (do_vPFC_fb){
  ...
  df <- df %>% 
    ...
    mutate(v_chosen_sc = scale(v_chosen),
           ...
           v_entropy_wi_change_bin_lag = case_when(
           ...
           v_entropy_wi_change_bin_lag2 = case_when(
             v_entropy_wi_change_lag2 < -0.5 ~ 'Decrease',
             v_entropy_wi_change_lag2 > 0.5  ~ 'Increase',
             v_entropy_wi_change_lag2 >= -0.5 & v_entropy_wi_change_lag2 <= 0.5 ~ 'No Change'
           ),
  ...
  df <- df %>% select(id,run,run_trial,v_entropy_wi_change_lag,rt_vmax_change_bin,v_entropy_wi_change,v_entropy_wi_change_bin,v_entropy_wi_change_bin_lag,v_entropy_wi_change_bin_lag2,trial_bin,iti_ideal,iti_prev,rt_csv,trial_neg_inv_sc,rt_csv_sc,rewFunc,v_entropy_sc,outcome,last_outcome, v_max_wi,score_sc,rt_bin,iti_sc,ev_sc,expl_longer,expl_shorter)
  ...
  Q1 <- Q %>% group_by(id,run,run_trial,network) %>% summarize(mD = mean(vmPFC_decon,na.rm=TRUE)) %>% ungroup()
  
  Q <- merge(Q,Q1,by=c('id','run','run_trial','network'))
  
  
  rm(decode_formula)
  decode_formula <- formula(~ (1|id))
  decode_formula[[1]] = formula(~ age + female + v_entropy_wi_change + outcome + trial_neg_inv_sc + rt_csv_sc + iti_sc + (1|id/run))
  decode_formula[[2]] = formula(~ age + female + v_entropy_wi_change + outcome + trial_neg_inv_sc + rt_csv_sc + iti_sc + last_outcome + (1|id/run))
  decode_formula[[3]] = formula(~ age + female + v_entropy_wi_change + outcome*v_entropy_wi_change + trial_neg_inv_sc + rt_csv_sc + iti_sc + (1+ mD|id/run))
  ...
  if (do_network){
    ...
    for (i in 3){
      ...
      save(ddf,file=paste0(curr_date,'-vmPFC-network-feedback-dHtest-',i,'.Rdata'))
    }
  }
}

Q1 <- Q %>% group_by(id,v_entropy_wi_change_bin,v_entropy_wi_change_bin_lag,v_entropy_wi_change_bin_lag2,evt_time,network) %>% summarize(mD = mean(vmPFC_decon,na.rm=TRUE))
Q2 <- Q1 %>% group_by(v_entropy_wi_change_bin,v_entropy_wi_change_bin_lag,v_entropy_wi_change_bin_lag2,evt_time,network) %>% summarize(mmD = mean(mD,na.rm=TRUE),dmD = sd(mD,na.rm=TRUE)/sqrt(length(mD)))

Q1 <- Q1 %>% filter(!is.na(v_entropy_wi_change_bin))
Q2 <- Q2 %>% filter(!is.na(v_entropy_wi_change_bin))
ggplot(Q1,aes(x=evt_time,y=mD,color=v_entropy_wi_change_bin)) + geom_smooth() + facet_wrap(~network)
