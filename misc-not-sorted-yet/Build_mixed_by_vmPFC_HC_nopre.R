...
if (do_vPFC_fb){
  ...
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      save(ddf,file=paste0(curr_date,'-vmPFC-symmetry-feedback-nopre-',i,'.Rdata'))
    }
  }
  if (do_network){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-network-feedback-nopre-',i,'.Rdata'))
    }
  }
}
###################################
...
if (do_vPFC_clock){
  ...
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-symmetry-clock-nopre-',i,'.Rdata'))
    }
  }
  if (do_network){
    splits = c('evt_time','network')
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-network-clock-nopre-',i,'.Rdata'))
    }
  }
}
#################################
...
if (do_HC_fb){
  ...
  for (i in 1:length(decode_formula)){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-HC-axis-feedback-nopre-',i,'.Rdata'))
  }
}
###################################
#####      HC- clock          #####
...
if (do_HC_clock){
  ...
  for (i in 1:length(decode_formula)){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-HC-axis-clock-nopre-',i,'.Rdata'))
  }
}
####################################
...
if (do_anat_fb){
  ...
  if (do_network){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-clock-anatomy-nopre-',i,'.Rdata'))
    }
  }
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-symmetry-clock-anatomy-nopre-',i,'.Rdata'))
    }
  }
}
...
if (do_anat_clock){
  ...
  df <- df %>% select(id,run,run_trial,outcome,trial_neg_inv_sc,iti_ideal,iti_prev,rt_csv,rt_csv_sc,iti_sc,rt_csv_sc,rewFunc,iti_lag_sc)
  ...
  if (do_network){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-feedback-anatomy-nopre-',i,'.Rdata'))
    }
  }
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-symmetry-feedback-anatomy-nopre-',i,'.Rdata'))
    }
  }
}
...
if (do_HC2vPFC_fb){
  ...
  if (do_network){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-feedback-nopre-',i,'.Rdata'))
    }
  }
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-symmetry-feedback-nopre-',i,'.Rdata'))
    }
  }
}
###################################
...
if (do_HC2vPFC_clock){
  ...
  #df <- df %>% filter(!is.na(rt_vmax_change_bin) | !is.na(v_entropy_wi_change_lag_bin))
  df <- df %>% select(id,run,run_trial,outcome,v_entropy_wi,iti_ideal,iti_prev,rt_csv,abs_pe_max_lag_sc,v_entropy_wi,rt_vmax_change_sc,trial_bin,rewFunc,trial_neg_inv_sc,rt_csv_sc,v_entropy_sc,expl_longer,expl_shorter,rt_bin,trial_bin,last_outcome,v_max_wi,v_entropy_wi_change_lag,score_lag_sc,iti_sc,iti_lag_sc,ev_lag_sc)
  Q <- merge(df, Q, by = c("id", "run", "run_trial")) %>% arrange("id","run","run_trial","evt_time")
  Q$vmPFC_decon[Q$evt_time > Q$rt_csv + Q$iti_ideal] = NA;
  Q$vmPFC_decon[Q$evt_time < -(Q$iti_prev)] = NA;
  Q$decon_mean[Q$evt_time > Q$rt_csv + Q$iti_ideal] = NA;
  ...
  if (do_network){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-clock-nopre-',i,'.Rdata'))
    }
  }
  ...
  if (do_symmetry){
    ...
    for (i in 1:length(decode_formula)){
      ...
      curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
      save(ddf,file=paste0(curr_date,'-vmPFC-HC-symmetry-clock-nopre-',i,'.Rdata'))
    }
  }
}
...
if (doTesting){
  ...
  for (i in 1:3){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-vmPFC-HC-network-testing-trxH-nopre-',i,'.Rdata'))
  }
  ...
  for (i in 1){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-vmPFC-HC-feedback-network-outcome-nopre-',i,'.Rdata'))
  }
}