plot_mixed_by_vmPFC_by_outcome <- function(ddf,toalign,toprocess,totest,behavmodel,model_iter){
  
  ## Check plots
  if (strcmp(toalign,"clock")){
    setwd('~/vmPFC/MEDUSA Schaefer Analysis/validate_mixed_by_clock_by_outcome')
  } else if (strcmp(toalign,"feedback")){
    setwd('~/vmPFC/MEDUSA Schaefer Analysis/validate_mixed_by_feedback_by_outcome')
  ...
  for (fe in terms) {
    ...
    if (strcmp(toprocess,"network")){
      ...
      gg1 <- ggplot(edf, aes(x=t, y=estimate, ymin=estimate-std.error, ymax=estimate+std.error, alpha=`p, FDR-corrected`)) +
        ...
        geom_vline(xintercept = 0, lty = "dashed", color = "#808080", size = 1) + facet_wrap(outcome1~symmetry_group1) + 
        xlab(epoch_label) +
