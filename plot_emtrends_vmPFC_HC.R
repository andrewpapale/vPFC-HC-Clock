...
plot_emtrends_vmPFC_HC <- function(ddf,toalign,toprocess,totest,behavmodel,model_iter,hc_LorR){
...
if (!strcmp(totest,'final-model-testing-')){
  ...
  tryCatch({
    ...
    fname = paste(behavmodel,'-',totest,"_",toalign, "_emtrends_", toprocess, "_", 'Peaks','-',hc_LorR, ".pdf", sep = "")
    pdf(fname, width = 9, height = 3.5)
    gg1 <- ggplot(emt,aes(x=evt_time,y=HCwithin.trend)) + 
      facet_grid(~HC_region) +
