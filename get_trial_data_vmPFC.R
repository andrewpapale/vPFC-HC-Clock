...
get_trial_data_vmPFC <- function(repo_directory=NULL, dataset="mmclock_fmri", groupfixed=TRUE) {
  ...
  full <- as_tibble(full) %>%
    dplyr::select(c(id, run, trial, v_entropy, rt_vmax, pe_max,v_max)) %>%
    mutate(rt_vmax = rt_vmax / 10) %>% # put into seconds
    dplyr::rename(v_entropy_full = v_entropy, rt_vmax_full = rt_vmax, pe_max_full = pe_max, v_max_full = v_max) %>%
    group_by(id, run) %>%
    mutate(
      ...
      v_max_wi_full = scale(v_max_full)
    ) %>%
    ungroup()
