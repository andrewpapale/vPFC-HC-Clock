...
if (do_rand_slopes){
  ...
  decode_formula <- NULL
  decode_formula[[1]] = formula(~ age + last_outcome + run_trial0_neg_inv_sc + rt_lag_sc + iti_lag_sc + (1 + v_entropy_wi|id) + (1|run))
  ...
  for (i in 1:length(decode_formula)){
    ...
    save(ddf,file=paste0(curr_date,'-vPFC-network-',toalign,'-Explore-ranslopes-HConly-trial_mod-trial1-10included-nofixedeffect-',i,'.Rdata'))
  }
}
    ...
    for (j in 1:length(decode_formula)){
      if (!simple_model && !trial_mod_model){
        ...
        if (j==1){
          ...
          m_dmn <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_dmn <- anova(m_dmn)
          zq <- Q2 %>% filter(network=='CTR')
          m_ctr <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_ctr <- anova(m_ctr)
          zq <- Q2 %>% filter(network=='LIM')
          m_lim <- lmer(rt_csv_sc ~(rt_lag_sc + subj_level_rand_slope + last_outcome)^2 + rt_lag_sc:last_outcome:subj_level_rand_slope + rt_vmax_lag_sc * subj_level_rand_slope*run_trial0_neg_inv_sc + (1 | id/run), data=zq)
          am_lim <- anova(m_lim)
          save(am_dmn,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-Explore-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-dmn-anova-',i,'.Rdata'))
          save(am_ctr,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-Explore-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-ctr-anova-',i,'.Rdata'))
          save(am_lim,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-Explore-pred-rt_csv_sc-int-notimesplit-nofixedeffect-rtvmax_by_trial-lim-anova-',i,'.Rdata'))
          save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-Explore-pred-rt_csv_sc-int-HConly-trial1-10included-notimesplit-nofixedeffect-',i,'.Rdata'))
        } else {
          save(ddq,file=paste0(curr_date,'-vmPFC-network-ranslopes-',toalign,'-Explore-pred-rt_csv_sc-slo-HConly-trial1-10included-notimesplit-nofixedeffect-',i,'.Rdata'))
        }
      } else if (simple_model){
        ddq <- mixed_by(Q, outcomes = "rt_csv_sc", rhs_model_formulae = decode_formula[[j]], split_on = splits,return_models=TRUE,
                        padjust_by = "term", padjust_method = "fdr", ncores = ncores, refit_on_nonconvergence = 3,
