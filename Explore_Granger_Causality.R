...
if (do_vPFC_HC){
  ...
  decode_formula[[1]] = formula(~ HCwithin*group + vmPFC_lag1*group + (1 | id/run))    
  decode_formula[[2]] = formula(~ HCwithin*group + vmPFC_lag2*group + (1 | id/run))    
  decode_formula[[3]] = formula(~ HCwithin*group + vmPFC_lag3*group + (1 | id/run))
  decode_formula[[4]] = formula(~ vmPFC_lag1*group + (1|id/run))
  decode_formula[[5]] = formula(~ vmPFC_lag2*group + (1|id/run))
  decode_formula[[6]] = formula(~ vmPFC_lag3*group + (1|id/run))
  ...
  for (i in 1:length(decode_formula)){
    ...
    save(ddf,file=paste0(curr_date,'-Explore-Granger-Causality-pred-vmPFC-network-clock-with-group2-',i,'.Rdata'))
  }
  
  
  rm(decode_formula)
  decode_formula <- NULL
  decode_formula[[1]] = formula(~ vmPFC_decon*group + HC_lag1*group + (1 | id/run))    
  decode_formula[[2]] = formula(~ vmPFC_decon*group + HC_lag2*group + (1 | id/run))    
  decode_formula[[3]] = formula(~ vmPFC_decon*group + HC_lag3*group + (1 | id/run))
  decode_formula[[4]] = formula(~ HC_lag1*group + (1 | id/run))
  decode_formula[[5]] = formula(~ HC_lag2*group + (1 | id/run))
  decode_formula[[6]] = formula(~ HC_lag3*group + (1 | id/run))
  qT <- c(-0.7,0.43)
  ...
  for (i in 1:length(decode_formula)){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Explore-Granger-Causality-pred-HC-network-clock-with-group2-',i,'.Rdata'))
  }
...
if (do_vPFC_HC_fb){
  ...
  for (i in 1:length(decode_formula)){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Explore-Granger-Causality-pred-vmPFC-network-feedback-with-group2-',i,'.Rdata'))
  }
  ...
  for (i in 1:length(decode_formula)){
    ...
    curr_date <- strftime(Sys.time(),format='%Y-%m-%d')
    save(ddf,file=paste0(curr_date,'-Explore-Granger-Causality-pred-HC-network-feedback-with-group2-',i,'.Rdata'))
  }
